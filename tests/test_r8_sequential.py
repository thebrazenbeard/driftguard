...
class SequentialCusumTests(unittest.TestCase):
    ...
    def test_superseded_subject_epoch_invalidates_detector(self):
        ...
        transition = SubjectEpochTransition(
            transition_id="r8-supersede",
            subject_id=self.subject.subject_id,
            predecessor_epoch=self.subject.epoch,
            predecessor_digest=self.subject.configuration_digest,
            successor_epoch=next_subject.epoch,
            successor_digest=next_subject.configuration_digest,
            reason="R8 superseded-epoch detector test",
        )
        self.ledger.transition_subject_epoch(
            predecessor=self.subject,
            successor=next_subject,
