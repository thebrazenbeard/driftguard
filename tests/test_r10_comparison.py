...
class DetectorComparisonTests(unittest.TestCase):
    ...
    def test_candidate_result_cannot_claim_fail_without_family_failures(self):
        spec = cusum_spec()
        c = corpus()
        cal = calibration_plan(c, spec)
        receipt = compare_detectors(
        ...
        genuine = receipt.candidate_results[0]
        with self.assertRaisesRegex(
            ValueError,
            "FAIL disposition requires family failures",
        ):
            DetectorCandidateResult(
                candidate_id=genuine.candidate_id,
                algorithm=genuine.algorithm,
                candidate_digest=genuine.candidate_digest,
                disposition=CalibrationDisposition.FAIL,
                family_metrics=genuine.family_metrics,
                reasons=("fabricated_failure",),
            )

    def test_comparison_receipt_cannot_be_directly_constructed(self):
        spec = cusum_spec()
        c = corpus()
        cal = calibration_plan(c, spec)
        genuine = compare_detectors(
            plan=comparison_plan(c, cal, candidates(spec)),
            calibration_plan=cal,
            corpus=c,
            cusum_spec=spec,
        ...
        with self.assertRaisesRegex(
            ValueError,
            "must come from compare_detectors",
        ):
            DetectorComparisonReceipt(
                comparison_plan_digest=genuine.comparison_plan_digest,
                calibration_plan_digest=genuine.calibration_plan_digest,
                corpus_digest=genuine.corpus_digest,
                corpus_role=genuine.corpus_role,
                disposition=DetectorComparisonDisposition.ONE_CANDIDATE_QUALIFIED,
                candidate_results=genuine.candidate_results,
                qualified_candidate_ids=("fabricated-winner",),
                descriptive_pareto_candidate_ids=("fabricated-winner",),
